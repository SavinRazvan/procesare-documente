#!/usr/bin/env python3
"""
Enclosure Search Processor
==========================

Professional GeoJSON processor for enclosure search files with comprehensive data validation, duplicate detection, and centralized output generation.

Features:
- ENCLOSURE_ID validation and duplicate detection
...
@dataclass
class EnclosureSearchFeature:
    """Represents a processed enclosure_search feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class EnclosureSearchProcessor:
    """
    Individual processor for enclosure_search files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the enclosure_search processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of EnclosureSearchFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        ...
        # Get key fields for enclosure_search
        key_fields = ["ENCLOSURE_ID", "LOCALITATE"]
    ...
    def _is_duplicate_feature(self, feature: Dict[str, Any]) -> bool:
        ...
        # First check if feature has valid enclosure data
        properties = feature.get('properties', {})
        enclosure_id = properties.get('ENCLOSURE_ID', '').strip()
        localitate = properties.get('LOCALITATE', '').strip()
        
        # Skip features with empty or invalid enclosure data
        if not enclosure_id or enclosure_id == '':
            self.logger.debug(f"Skipping feature with empty ENCLOSURE_ID: {properties}")
            return True
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with enclosure_search model"""
        import time
        ...
        try:
            ...
            if not features:
                return ProcessingResult(
                    success=True,
                    model_detected="enclosure_search",
            ...
            # Process with enclosure_search model
            processed_features = []
            errors = []
            warnings = []
            ...
            for feature in features:
                try:
                    # Extract fields according to enclosure_search model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("enclosure_search", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single enclosure_search file"""
        self.logger.info(f"Processing enclosure_search file: {file_path}")
        
        try:
            # Check if file has ENCLOSURE_ in the name
            file_name = Path(file_path).name
            if "ENCLOSURE_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'ENCLOSURE_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="enclosure_search",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'ENCLOSURE_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'ENCLOSURE_' in name"}
                )
            ...
            # Check if file has enclosure_search required fields
            enclosure_required_fields = {"ENCLOSURE_ID", "LOCALITATE"}
            file_fields_set = set(field.upper() for field in field_names)
            
            if not enclosure_required_fields.issubset(file_fields_set):
                self.logger.warning(f"File {file_path} does not have required enclosure_search fields: {enclosure_required_fields}")
                return ProcessingResult(
                    success=False,
                    model_detected="enclosure_search",
                    features_processed=0,
                    features_extracted=0,
                    errors=["Missing required enclosure_search fields"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "Missing required enclosure_search fields"}
                )
            
            # Process the file with enclosure_search model
            ...
            for feature in features:
                ...
                # Check if required fields for enclosure_search are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["ENCLOSURE_ID", "LOCALITATE"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for enclosure_search: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = EnclosureSearchFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized enclosure_search file"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "enclosure_search.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Enclosure Search Data",\n')
            f.write('"features": [\n')
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    enclosure_id = properties.get("ENCLOSURE_ID", "")
                    localitate = properties.get("LOCALITATE", "")
                    
                    feature_str = '{ "type": "Feature", "properties": { "ENCLOSURE_ID": "' + json.dumps(enclosure_id)[1:-1] + '", "LOCALITATE": "' + json.dumps(localitate)[1:-1] + '" }, "geometry": '
                    ...
                    self.logger.warning(f"Failed to process Enclosure entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized enclosure_search file: {output_file} ({len(self.centralized_data)} features)")
        return str(output_file)
    
    def get_processing_summary(self) -> Dict[str, Any]:
        """Get processing summary"""
        return {
            "model_type": "enclosure_search",
...
def main():
    """Main function for enclosure_search processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process enclosure_search files individually and create centralized output")
    parser.add_argument("input_dir", help="Input directory containing enclosure_search files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = EnclosureSearchProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all enclosure_search files
    ...
    print("\n🎉 Enclosure Search Processing Complete!")
