#!/usr/bin/env python3
"""
Spliter Individual Processor
============================

Professional GeoJSON processor for spliter files with comprehensive data validation, duplicate detection, and centralized output generation.

Features:
- Spliter ID validation and duplicate detection
...
@dataclass
class SpliterFeature:
    """Represents a processed spliter feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class SpliterProcessor:
    """
    Individual processor for spliter files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the spliter processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of SpliterFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        """Generate a unique hash for a feature based on TIP_SPLITER and geometry"""
        import hashlib
        
        # For Spliter, uniqueness is based on TIP_SPLITER + geometry coordinates
        # This ensures we don't have duplicate features at the same location
        key_fields = ["TIP_SPLITER"]
        ...
        # Add geometry hash for uniqueness
        geometry = feature.get("geometry", {})
        geometry_str = json.dumps(geometry, sort_keys=True)
        
        # Combine key values and geometry for uniqueness
    ...
    def _is_duplicate_feature(self, feature: Dict[str, Any]) -> bool:
        ...
        # For spliter, the key field is TIP_SPLITER only
        properties = feature.get('properties', {})
        tip_spliter = properties.get('TIP_SPLITER', '').strip()
        
        # Skip features with empty TIP_SPLITER (this is the required field for spliter)
        if not tip_spliter or tip_spliter == '':
            self.logger.debug(f"Skipping feature with empty TIP_SPLITER: {properties}")
            return True
        
        # For spliter, we don't want to skip duplicates based on TIP_SPLITER alone
        # Multiple spliter features can have the same TIP_SPLITER value
        # We only check for exact duplicates (same feature hash)
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with spliter model"""
        import time
        ...
        try:
            ...
            for feature in features:
                try:
                    # Extract fields according to spliter model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("spliter", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single spliter file"""
        self.logger.info(f"Processing spliter file: {file_path}")
        
        try:
            # Check if file has SPLITER_ in the name
            file_name = Path(file_path).name
            if "SPLITER_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'SPLITER_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="spliter",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'SPLITER_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'SPLITER_' in name"}
                )
            ...
            # Check if it's a spliter file
            if result.model_detected != "spliter":
                self.logger.warning(f"File {file_path} is not a spliter file (detected: {result.model_detected})")
                return result
            ...
            for feature in features:
                ...
                # Check if required fields for spliter are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["TIP_SPLITER"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for spliter: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = SpliterFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized spliter file in compact format"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "spliter_centralized.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Spliter Data",\n')
            f.write('"features": [\n')
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    self.logger.warning(f"Failed to process spliter entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized spliter file: {output_file} ({len(self.centralized_data)} features)")
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_spliter_types": len(self.fttb_code_tracking)
        }

def main():
    """Main function for spliter processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process spliter files and create centralized output only")
    parser.add_argument("input_dir", help="Input directory containing spliter files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = SpliterProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all spliter files
    ...
    print(f"🔑 Spliter duplicates: {summary['duplicate_stats']['fttb_code_duplicates']}")
    print(f"✅ Unique spliter types: {summary['unique_spliter_types']}")
    if centralized_file:
