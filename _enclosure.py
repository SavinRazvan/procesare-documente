#!/usr/bin/env python3
"""
Enclosure Individual Processor
=============================

Professional GeoJSON processor for enclosure files with comprehensive data validation,
duplicate detection, and centralized output generation.
...
@dataclass
class EnclosureFeature:
    """Represents a processed enclosure feature"""
...
class EnclosureProcessor:
    """
    Individual processor for enclosure files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the enclosure processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of EnclosureFeature
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with enclosure model"""
        import time
        ...
        try:
            ...
            for feature in features:
                try:
                    # Extract fields according to enclosure model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("enclosure", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single enclosure file"""
        self.logger.info(f"Processing enclosure file: {file_path}")
        ...
        try:
            ...
            # Check if it's an enclosure file
            if result.model_detected != "enclosure":
                self.logger.warning(f"File {file_path} is not an enclosure file (detected: {result.model_detected})")
                return result
            ...
            for feature in features:
                ...
                # Check if required fields for enclosure are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["ENCLOSURE_ID", "LOCALITATE", "OBSERVATII"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for enclosure: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = EnclosureFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized enclosure file in compact format"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "enclosure_centralized.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Enclosure Data",\n')
            f.write('"features": [\n')
        ...
        self.logger.info(f"Saved centralized enclosure file: {output_file} ({len(self.centralized_data)} features)")
...
def main():
    """Main function for enclosure processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process enclosure files and create centralized output only")
    parser.add_argument("input_dir", help="Input directory containing enclosure files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = EnclosureProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all enclosure files
