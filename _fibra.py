#!/usr/bin/env python3
"""
Fibra Individual Processor
==========================

Professional GeoJSON processor for fibra files with comprehensive data validation, duplicate detection, and centralized output generation.

Features:
- Fibra ID validation and duplicate detection
...
@dataclass
class FibraFeature:
    """Represents a processed fibra feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class FibraProcessor:
    """
    Individual processor for fibra files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the fibra processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of FibraFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        ...
        # Get key fields for fibra
        key_fields = ["FIBRA_ID", "LOCALITATE"]
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with fibra model"""
        import time
        ...
        try:
            ...
            for feature in features:
                try:
                    # Extract fields according to fibra model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("fibra", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single fibra file"""
        self.logger.info(f"Processing fibra file: {file_path}")
        
        try:
            # Check if file has FO_ in the name
            file_name = Path(file_path).name
            if "FO_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'FO_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="fibra",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'FO_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'FO_' in name"}
                )
            ...
            # Check if it's a fibra file
            if result.model_detected != "fibra":
                self.logger.warning(f"File {file_path} is not a fibra file (detected: {result.model_detected})")
                return result
            ...
            for feature in features:
                ...
                # Check if required fields for fibra are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["NR_FIRE", "LUNGIME_HARTA", "LUNGIME_ESTIMATA", "TIP_CABLU", "LUNGIME_TEREN", "LUNGIME_OPTICA", "AMPLASARE", "LOCALITATE"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for fibra: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = FibraFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized fibra file in compact format"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "fibra_centralized.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Fibra Data",\n')
            f.write('"features": [\n')
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    self.logger.warning(f"Failed to process fibra entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized fibra file: {output_file} ({len(self.centralized_data)} features)")
...
def main():
    """Main function for fibra processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process fibra files and create centralized output only")
    parser.add_argument("input_dir", help="Input directory containing fibra files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = FibraProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all fibra files
