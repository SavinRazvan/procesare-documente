#!/usr/bin/env python3
"""
Localitati Individual Processor
===============================

Professional GeoJSON processor for localitati files with comprehensive data validation, duplicate detection, and centralized output generation.

Features:
- Localitati ID validation and duplicate detection
...
@dataclass
class LocalitatiFeature:
    """Represents a processed localitati feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class LocalitatiProcessor:
    """
    Individual processor for localitati files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the localitati processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of LocalitatiFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        ...
        # Get key fields for localitati
        key_fields = ["NUME", "COMUNA"]
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with localitati model"""
        import time
        ...
        try:
            ...
            for feature in features:
                try:
                    # Extract fields according to localitati model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("localitati", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single localitati file"""
        self.logger.info(f"Processing localitati file: {file_path}")
        
        try:
            # Check if file has LOCALITATI_ in the name
            file_name = Path(file_path).name
            if "LOCALITATI_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'LOCALITATI_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="localitati",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'LOCALITATI_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'LOCALITATI_' in name"}
                )
            ...
            # Check if it's a localitati file
            if result.model_detected != "localitati":
                self.logger.warning(f"File {file_path} is not a localitati file (detected: {result.model_detected})")
                return result
            ...
            for feature in features:
                ...
                # Check if required fields for localitati are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["NUME", "COMUNA", "NR_CASE", "TIP_RETEA_CASE", "HUB", "NR_PONI", "TIP_PONI", "IMPLEMENTARE_RETEA", "STATIE_CATV", "HP_TOTAL", "SIRUTA", "OBS", "PROIECTANT"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for localitati: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = LocalitatiFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized localitati file in compact format"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "localitati_centralized.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Localitati Data",\n')
            f.write('"features": [\n')
            
            # Sort entries alphabetically by NUME
            sorted_entries = sorted(self.centralized_data, key=lambda entry: entry.feature.get("properties", {}).get("NUME", "").lower())
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    self.logger.warning(f"Failed to process localitati entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized localitati file: {output_file} ({len(self.centralized_data)} features)")
        return str(output_file)
    
    def get_processing_summary(self) -> Dict[str, Any]:
        """Get processing summary"""
        return {
            "model_type": "localitati",
...
def main():
    """Main function for localitati processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process localitati files and create centralized output only")
    parser.add_argument("input_dir", help="Input directory containing localitati files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = LocalitatiProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all localitati files
    ...
    print("\n🎉 Localitati Processing Complete!")
