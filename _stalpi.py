#!/usr/bin/env python3
"""
Stalpi Individual Processor
===========================

Professional GeoJSON processor for stalpi files with comprehensive data validation, duplicate detection, and centralized output generation.

Features:
- Stalpi ID validation and duplicate detection
...
@dataclass
class StalpiFeature:
    """Represents a processed stalpi feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class StalpiProcessor:
    """
    Individual processor for stalpi files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the stalpi processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of StalpiFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        ...
        # Get key fields for stalpi
        key_fields = ["STALP_ID", "LOCALITATE"]
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with stalpi model"""
        import time
        ...
        try:
            ...
            for feature in features:
                try:
                    # Extract fields according to stalpi model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("stalpi", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single stalpi file"""
        self.logger.info(f"Processing stalpi file: {file_path}")
        
        try:
            # Check if file has STALPI_ in the name
            file_name = Path(file_path).name
            if "STALPI_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'STALPI_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="stalpi",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'STALPI_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'STALPI_' in name"}
                )
            ...
            # Check if it's a stalpi file
            if result.model_detected != "stalpi":
                self.logger.warning(f"File {file_path} is not a stalpi file (detected: {result.model_detected})")
                return result
            ...
            for feature in features:
                ...
                # Check if required fields for stalpi are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["LOCALITATE", "COD_FTTB", "DENUMIRE_ART", "NR_ART", "FOLOSIT_RDS", "MATERIAL_CONSTRUCTIV", "TIP_STALP", "PROPRIETAR", "TABELA"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for stalpi: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = StalpiFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized stalpi file in compact format"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "stalpi_centralized.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Stalpi Data",\n')
            f.write('"features": [\n')
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    self.logger.warning(f"Failed to process stalpi entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized stalpi file: {output_file} ({len(self.centralized_data)} features)")
...
def main():
    """Main function for stalpi processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process stalpi files and create centralized output only")
    parser.add_argument("input_dir", help="Input directory containing stalpi files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = StalpiProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all stalpi files
