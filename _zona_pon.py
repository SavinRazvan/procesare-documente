#!/usr/bin/env python3
"""
Zona PON Individual Processor
=============================

Professional GeoJSON processor for zona PON files with comprehensive data validation, duplicate detection, and centralized output generation.
...
@dataclass
class ZonaPonFeature:
    """Represents a processed zona_pon feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class ZonaPonProcessor:
    """
    Individual processor for zona_pon files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the zona_pon processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of ZonaPonFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        ...
        # Get key fields for zona_pon
        key_fields = ["ZONA_PON_ID", "LOCALITATE"]
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with zona_pon model"""
        import time
        ...
        try:
            ...
            for feature in features:
                try:
                    # Extract fields according to zona_pon model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("zona_pon", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single zona_pon file"""
        self.logger.info(f"Processing zona_pon file: {file_path}")
        
        try:
            # Check if file has ZONA_PON_REALIZAT_ in the name
            file_name = Path(file_path).name
            if "ZONA_PON_REALIZAT_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'ZONA_PON_REALIZAT_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="zona_pon",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'ZONA_PON_REALIZAT_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'ZONA_PON_REALIZAT_' in name"}
                )
            ...
            # Check if it's a zona_pon file
            if result.model_detected != "zona_pon":
                self.logger.warning(f"File {file_path} is not a zona_pon file (detected: {result.model_detected})")
                return result
            ...
            for feature in features:
                ...
                # Check if required fields for zona_pon are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["MI_PRINX", "NR_ABONATI", "OBSERVATII", "PON"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for zona_pon: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = ZonaPonFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized zona_pon file in compact format"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "zona_pon_centralized.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Zona PON Data",\n')
            f.write('"features": [\n')
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    self.logger.warning(f"Failed to process zona_pon entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized zona_pon file: {output_file} ({len(self.centralized_data)} features)")
...
def main():
    """Main function for zona_pon processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process zona_pon files and create centralized output only")
    parser.add_argument("input_dir", help="Input directory containing zona_pon files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = ZonaPonProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all zona_pon files
