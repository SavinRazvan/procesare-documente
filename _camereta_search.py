#!/usr/bin/env python3
"""
Camereta Search Processor
=========================

Professional GeoJSON processor for camereta search files with comprehensive data validation, duplicate detection, and centralized output generation.

Features:
- ID_TABELA validation and duplicate detection
...
@dataclass
class CameretaSearchFeature:
    """Represents a processed camereta_search feature"""
    feature: Dict[str, Any]
    source_file: str
    processing_timestamp: str

class CameretaSearchProcessor:
    """
    Individual processor for camereta_search files with standardized headers
    """
    
    def __init__(self, enable_duplicate_detection: bool = True):
        """Initialize the camereta_search processor"""
        self.processor = GeoJSONProcessor()
        self.enable_duplicate_detection = enable_duplicate_detection
        self.centralized_data = []  # List of CameretaSearchFeature
    ...
    def _generate_feature_hash(self, feature: Dict[str, Any]) -> str:
        ...
        # Get key fields for camereta_search
        key_fields = ["LOCALITATE", "ID_TABELA"]
    ...
    def _is_duplicate_feature(self, feature: Dict[str, Any]) -> bool:
        ...
        # First check if feature has valid camereta data
        properties = feature.get('properties', {})
        localitate = properties.get('LOCALITATE', '').strip()
        id_tabela = properties.get('ID_TABELA', '').strip()
        
        # Skip features with empty or invalid camereta data
        if not localitate or localitate == '':
            self.logger.debug(f"Skipping feature with empty LOCALITATE: {properties}")
            return True
            
        if not id_tabela or id_tabela == '':
            self.logger.debug(f"Skipping feature with empty ID_TABELA: {properties}")
            return True
    ...
    def _process_loaded_data(self, data: Dict[str, Any], file_path: str) -> ProcessingResult:
        """Process already loaded data with camereta_search model"""
        import time
        ...
        try:
            ...
            if not features:
                return ProcessingResult(
                    success=True,
                    model_detected="camereta_search",
            ...
            # Process with camereta_search model
            processed_features = []
            errors = []
            warnings = []
            ...
            for feature in features:
                try:
                    # Extract fields according to camereta_search model
                    properties = feature.get('properties', {})
                    extracted_data = self.processor.model_manager.extract_fields("camereta_search", properties)
    ...
    def process_file(self, file_path: str, output_dir: str) -> ProcessingResult:
        """Process a single camereta_search file"""
        self.logger.info(f"Processing camereta_search file: {file_path}")
        
        try:
            # Check if file has CAMERETA_ in the name
            file_name = Path(file_path).name
            if "CAMERETA_" not in file_name.upper():
                self.logger.warning(f"File {file_path} does not contain 'CAMERETA_' in name, skipping")
                return ProcessingResult(
                    success=False,
                    model_detected="camereta_search",
                    features_processed=0,
                    features_extracted=0,
                    errors=["File does not contain 'CAMERETA_' in name"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "File does not contain 'CAMERETA_' in name"}
                )
            ...
            # Check if file has camereta_search required fields
            camereta_required_fields = {"LOCALITATE", "ID_TABELA"}
            file_fields_set = set(field.upper() for field in field_names)
            
            if not camereta_required_fields.issubset(file_fields_set):
                self.logger.warning(f"File {file_path} does not have required camereta_search fields: {camereta_required_fields}")
                return ProcessingResult(
                    success=False,
                    model_detected="camereta_search",
                    features_processed=0,
                    features_extracted=0,
                    errors=["Missing required camereta_search fields"],
                    warnings=[],
                    processing_time=0.0,
                    metadata={"error_message": "Missing required camereta_search fields"}
                )
            
            # Process the file with camereta_search model
            ...
            for feature in features:
                ...
                # Check if required fields for camereta_search are empty
                has_required_values = False
                empty_required_fields = []
                for field in ["LOCALITATE", "ID_TABELA", "TIP_CAMERETA", "DIGI_ID"]:
                ...
                if not has_required_values:
                    self.logger.debug(f"Skipping feature with empty required fields for camereta_search: {', '.join(empty_required_fields)}")
                    empty_features_skipped += 1
                    continue
            ...
            for feature in filtered_features:
                centralized_feature = CameretaSearchFeature(
                    feature=feature,
    ...
    def save_centralized_file(self, output_dir: str) -> str:
        """Save centralized camereta_search file"""
        if not self.centralized_data:
            self.logger.warning("No centralized data to save")
            return ""
        
        output_file = Path(output_dir) / "camereta_search.geojson"
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('"name": "Centralized Camereta Search Data",\n')
            f.write('"features": [\n')
            ...
            for i, entry in enumerate(sorted_entries):
                try:
                    ...
                    id_tabela = properties.get("ID_TABELA", "")
                    
                    feature_str = '{ "type": "Feature", "properties": { "LOCALITATE": "' + json.dumps(localitate)[1:-1] + '", "ID_TABELA": "' + json.dumps(id_tabela)[1:-1] + '" }, "geometry": '
                    ...
                    self.logger.warning(f"Failed to process Camereta entry: {e}")
                    continue
            
            f.write(']\n')
            f.write('}\n')
        
        self.logger.info(f"Saved centralized camereta_search file: {output_file} ({len(self.centralized_data)} features)")
        return str(output_file)
    
    def get_processing_summary(self) -> Dict[str, Any]:
        """Get processing summary"""
        return {
            "model_type": "camereta_search",
...
def main():
    """Main function for camereta_search processing"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Process camereta_search files individually and create centralized output")
    parser.add_argument("input_dir", help="Input directory containing camereta_search files")
    parser.add_argument("output_dir", help="Output directory for processed files")
    parser.add_argument("--no-duplicates", action="store_true", help="Disable duplicate detection")
    
    args = parser.parse_args()
    
    processor = CameretaSearchProcessor(enable_duplicate_detection=not args.no_duplicates)
    
    # Process all camereta_search files
    ...
    print("\n🎉 Camereta Search Processing Complete!")
